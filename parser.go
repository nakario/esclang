...
func (p *Parser) Parse() error {
	...
	for t, err = p.l.NextToken(); err == nil; t, err = p.l.NextToken() {
		...
		case NormalToken:
			...
			if labeling {
				Logger.Println("Create label:", string([]rune{token.Char()}))
				_, ok := p.labels[token.Char()]
				if ok {
					return errors.New("duplicated label")
				}
				p.labels[token.Char()] = len(p.instructions)
			}
			if bgInstruction != nil {
		...
		case _SGRToken:
			Logger.Println("SGR token:", token.param)
			if 30 <= token.param && token.param <= 39 {
				switch token.param {
				case 30:
			...
			} else if 40 <= token.param && token.param <= 49 {
				labeling = false
				jumping = false
				switch token.param {
				case 40:
